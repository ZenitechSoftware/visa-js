const { wrapToPromise, eachLimit } = require("./utils.cjs");
...
const evaluateObjects = (objectName, operationName, objects, refs) => {
  if (!Array.isArray(objects)) {
    return Promise.reject(
      new VisaError(
        `visa.js: object '${objectName}' operation '${operationName}': 'mapRefsToObjects' function should return array of objects`,
      ),
    );
  }
  ...
  if (
    objects.findIndex((object) => object === null || object === undefined) !==
    -1
  ) {
    return Promise.reject(new Unauthorized());
  }
...
const evaluateMapRefsToObjects = (
  objectName,
  operationName,
  mapRefsToObjects,
  args,
  ...
  if (!mapRefsToObjects) {
    return Promise.reject(
      new VisaError(
        `visa.js: object '${objectName}' operation '${operationName}': object references (${refs}) can not be resolved because 'mapRefsToObjects' method is not provided`,
      ),
    );
  }
  return wrapToPromise(mapRefsToObjects, refs)
    ...
    .then((objects) =>
      evaluateObjects(objectName, operationName, objects, refs),
    );
};

const evaluateRuleForObjects = (
  objectName,
  operationName,
  rule,
  subject,
  ...
  new Promise((resolve, reject) =>
    eachLimit(
      ...
      5,
      (object, cb) =>
        evaluateRule(objectName, operationName, rule, subject, object, context)
          .then(cb)
          .catch(cb),
      (error) => {
        if (error) {
...
const createEvaluationFunction = (
  rule,
  mapRefsToObjects,
  objectName,
  operationName,
  ...
  function (args) {
    const result =
      args && (args.ref || args.refs || args.object || args.objects)
        ? evaluateRulesForObjects(
            objectName,
            operationName,
            rule,
            this.subject,
...
const createPolicyFunction = (Can) => (policy) => {
  for (let objectName of Object.keys(policy.objects)) {
    for (let operationName of Object.keys(
      policy.objects[objectName].operations,
    )) {
      ...
      Can.prototype[`_${operationName}Class`].prototype[objectName] =
