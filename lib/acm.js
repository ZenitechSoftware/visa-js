...
const evaluateRule = (objectName, operationName, rule, subject, object, context) => wrapToPromise(rule, subject, object, context)
  .catch(error => {
...
const evaluateRuleForObjects = (objectName, operationName, rule, subject, objects, context) => {
  return new Promise((resolve, reject) => async.eachLimit(
    objects,
    5,
    (object, cb) => evaluateRule(objectName, operationName, rule, subject, object, context)
      .then(cb)
      .catch(cb),
    (error, authorized) => {
      if (error) {
...
const evaluateRulesForObjects = (objectName, operationName, rule, subject, mapRefsToObjects, args) =>
  evaluateMapRefsToObjects(objectName, operationName, rule, subject, mapRefsToObjects, args)
    .then(objects => {
      ...
      return evaluateRuleForObjects(objectName, operationName, rule, this.subject, objects, args.context)
    });
